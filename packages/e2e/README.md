...
`fixtures/*.pptx` are generated by `scripts/generate-fixtures.ts` (`pnpm
fixtures`) and committed so runs are deterministic. Each deck is minimal and
targets specific regressions, so see the header comment in the generator before
adding to one.

Ground-truth PNGs under `fixtures/ground-truth/` come from PowerPoint itself via
`pnpm oracle:export` (Windows, PowerPoint installed).
