...
export type { ChartFrameStyle } from "./chart/type";
...
function buildSmoothScatterLineData(data: number[][], stepsPerSegment = 24): number[][] {
  ...
  const n = data.length;
  const slopes = Array.from<number>({ length: n - 1 });
  for (let i = 0; i < n - 1; i++) {
    slopes[i] = (data[i + 1][1] - data[i][1]) / (data[i + 1][0] - data[i][0]);
  }
  const tangents = Array.from<number>({ length: n });
  tangents[0] = slopes[0];
...
function normalizePercentStackedValues(seriesArr: SeriesData[]): number[][] {
  ...
  const totals: number[] = Array.from({ length: pointCount }, () => 0);
