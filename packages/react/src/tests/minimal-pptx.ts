...
function getContentTypes(slideCount: number): string {
  return (
    XML_DECL +
...
function getPresentation(slideCount: number): string {
  return (
    XML_DECL +
    `<p:presentation xmlns:a="http://schemas.openxmlformats.org/drawingml/2006/main" xmlns:r="http://schemas.openxmlformats.org/officeDocument/2006/relationships" xmlns:p="http://schemas.openxmlformats.org/presentationml/2006/main">
<p:sldMasterIdLst><p:sldMasterId id="2147483648" r:id="rId1"/></p:sldMasterIdLst>
<p:sldIdLst>
${Array.from({ length: slideCount }, (_, i) => `<p:sldId id="${256 + i}" r:id="rId${2 + i}"/>`).join("\n")}
</p:sldIdLst>
...
function getPresentationRels(slideCount: number): string {
  return (
    XML_DECL +
    `<Relationships xmlns="http://schemas.openxmlformats.org/package/2006/relationships">
<Relationship Id="rId1" Type="http://schemas.openxmlformats.org/officeDocument/2006/relationships/slideMaster" Target="slideMasters/slideMaster1.xml"/>
${Array.from({ length: slideCount }, (_, i) => `<Relationship Id="rId${2 + i}" Type="http://schemas.openxmlformats.org/officeDocument/2006/relationships/slide" Target="slides/slide${i + 1}.xml"/>`).join("\n")}
...
function getSlideXml(index: number): string {
  return (
    XML_DECL +
...
<p:grpSpPr><a:xfrm><a:off x="0" y="0"/><a:ext cx="0" cy="0"/><a:chOff x="0" y="0"/><a:chExt cx="0" cy="0"/></a:xfrm></p:grpSpPr>
<p:sp>
<p:nvSpPr><p:cNvPr id="2" name="Rect ${index}"/><p:cNvSpPr/><p:nvPr/></p:nvSpPr>
<p:spPr>
<a:xfrm><a:off x="1219200" y="1219200"/><a:ext cx="2743200" cy="1828800"/></a:xfrm>
...
/**
 * Builds the fixture and returns it as an ArrayBuffer (like a fetched file).
 *
 * `slideCount` is only worth passing to cover degenerate decks, such as the
 * slide-less one the store has to survive.
 */
export async function buildMinimalPptx(slideCount = SLIDE_COUNT): Promise<ArrayBuffer> {
  const zip = new JSZip();
  zip.file("[Content_Types].xml", getContentTypes(slideCount));
  zip.file("_rels/.rels", rootRels);
  zip.file("ppt/presentation.xml", getPresentation(slideCount));
  zip.file("ppt/_rels/presentation.xml.rels", getPresentationRels(slideCount));
  ...
  for (let i = 1; i <= slideCount; i++) {
    zip.file(`ppt/slides/slide${i}.xml`, getSlideXml(i));
    zip.file(`ppt/slides/_rels/slide${i}.xml.rels`, slideRels);
  }
  return zip.generateAsync({ type: "arraybuffer" });
}

export const FIXTURE_SLIDE_COUNT = SLIDE_COUNT;
