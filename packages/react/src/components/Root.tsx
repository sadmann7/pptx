...
export function Root({ file, children, onLoad, onError }: RootProps) {
  ...
  React.useEffect(() => {
    if (!file) { store.reset(); return; }
    const input = file instanceof File ? (file as Blob) : file;
    store.load(input)
