...
export interface ThemeFontInfo { latin: string; ea: string; cs: string; scripts?: Record<string, string>; }
...
const COLOR_SLOTS = ['dk1','dk2','lt1','lt2','accent1','accent2','accent3','accent4','accent5','accent6','hlink','folHlink'] as const;
...
export function parseTheme(root: SafeXmlNode): ThemeData {
  ...
  for (const slot of COLOR_SLOTS) {
    ...
    if (node.exists()) { const hex = extractColor(node); if (hex) colorScheme.set(slot, hex); }
  }

  const fontScheme = scope.child('fontScheme');
  const parseFI = (n: SafeXmlNode): ThemeFontInfo => {
    const scripts: Record<string, string> = {};
    for (const f of n.children('font')) { const s = f.attr('script'), t = f.attr('typeface'); if (s && t) scripts[s] = t; }
    const r: ThemeFontInfo = { latin: n.child('latin').attr('typeface') ?? '', ea: n.child('ea').attr('typeface') ?? '', cs: n.child('cs').attr('typeface') ?? '' };
