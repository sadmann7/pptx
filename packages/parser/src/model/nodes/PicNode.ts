...
export function parsePicNode(picNode: SafeXmlNode): PicNodeData {
  ...
  if (srcRect.exists()) {
    const t = srcRect.numAttr('t'), b = srcRect.numAttr('b'), l = srcRect.numAttr('l'), r = srcRect.numAttr('r');
    if (t !== undefined || b !== undefined || l !== undefined || r !== undefined) {
      crop = { top: (t ?? 0) / 100000, bottom: (b ?? 0) / 100000, left: (l ?? 0) / 100000, right: (r ?? 0) / 100000 };
    }
  }
  ...
  return { ...base, nodeType: 'picture', blipEmbed, blipLink, crop, fill, line, presetGeometry, customGeometry, isVideo: isVideo || undefined, isAudio: isAudio || undefined, mediaRId };
}
