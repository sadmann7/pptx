...
export interface TextRun { text: string; properties?: SafeXmlNode; }
...
export interface LineEndInfo { type: string; w?: string; len?: string; }
export interface TextBoxBounds { x: number; y: number; w: number; h: number; rotation?: number; }
...
function parseParagraph(pNode: SafeXmlNode): TextParagraph {
  ...
  for (const child of pNode.allChildren()) {
    ...
    if (ln === 'r') {
      orderedRuns.push({ text: child.child('t').text(), properties: child.child('rPr').exists() ? child.child('rPr') : undefined });
    } else if (ln === 'br') {
  ...
  return { properties: pPr.exists() ? pPr : undefined, runs: orderedRuns, level, endParaRPr: endParaRPrNode.exists() ? endParaRPrNode : undefined };
}
...
export function parseTextBody(txBody: SafeXmlNode): TextBody | undefined {
  ...
  for (const pNode of txBody.children('p')) { paragraphs.push(parseParagraph(pNode)); }
  return { bodyProperties: bodyPr.exists() ? bodyPr : undefined, listStyle: lstStyle.exists() ? lstStyle : undefined, paragraphs };
}
...
function findFill(spPr: SafeXmlNode): SafeXmlNode | undefined {
  for (const ft of FILL_TYPES) { const f = spPr.child(ft); if (f.exists()) return f; }
  return undefined;
}
...
export function parseShapeNode(spNode: SafeXmlNode): ShapeNodeData {
  ...
  if (ln.exists()) {
    ...
    if (he.exists()) { const t = he.attr('type'); if (t && t !== 'none') headEnd = { type: t, w: he.attr('w'), len: he.attr('len') }; }
    const te = ln.child('tailEnd');
    if (te.exists()) { const t = te.attr('type'); if (t && t !== 'none') tailEnd = { type: t, w: te.attr('w'), len: te.attr('len') }; }
  }
  ...
  if (txXfrm.exists()) {
    const txOff = txXfrm.child('off'); const txExt = txXfrm.child('ext');
    const xfrm = spPr.child('xfrm'); const off = xfrm.child('off'); const ext = xfrm.child('ext');
    const shapeW = ext.numAttr('cx') ?? 0; const shapeH = ext.numAttr('cy') ?? 0;
    const txW = txExt.numAttr('cx') ?? 0; const txH = txExt.numAttr('cy') ?? 0;
    if (shapeW > 0 && shapeH > 0) {
      const shapeX = off.numAttr('x') ?? 0; const shapeY = off.numAttr('y') ?? 0;
      const txX = txOff.numAttr('x') ?? 0; const txY = txOff.numAttr('y') ?? 0;
      const txRotDeg = angleToDeg(txXfrm.numAttr('rot') ?? 0);
      const localX = txX - shapeX; const localY = txY - shapeY;
      ...
      textBoxBounds = {
        ...
        w: emuToPx(txW), h: emuToPx(txH), rotation: txRotDeg,
      };
    }
  }

  return { ...base, nodeType: 'shape', presetGeometry, adjustments, customGeometry, fill, line, headEnd, tailEnd, textBody, textBoxBounds };
}
