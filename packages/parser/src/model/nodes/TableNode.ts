...
export interface TableCell { gridSpan: number; rowSpan: number; hMerge: boolean; vMerge: boolean; textBody?: TextBody; properties?: SafeXmlNode; }
export interface TableRow { height: number; cells: TableCell[]; }
...
export function parseTableNode(frameNode: SafeXmlNode): TableNodeData {
  ...
  for (const gridCol of tblGrid.children('gridCol')) columns.push(emuToPx(gridCol.numAttr('w') ?? 0));
  ...
  if (!tableStyleId) { const ts = tblPr.child('tblStyle'); if (ts.exists()) tableStyleId = ts.attr('val') ?? (ts.text() || undefined); }
  if (!tableStyleId) tableStyleId = tblPr.attr('tblStyle');

  return { ...base, nodeType: 'table', columns, rows, properties: tblPr.exists() ? tblPr : undefined, tableStyleId };
}
