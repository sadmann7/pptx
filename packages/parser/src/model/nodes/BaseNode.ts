...
export interface PlaceholderInfo { type?: string; idx?: number; }
export interface HlinkAction { action?: string; rId?: string; tooltip?: string; }
...
function findXfrm(node: SafeXmlNode): SafeXmlNode {
  ...
  if (spPr.exists()) { const x = spPr.child('xfrm'); if (x.exists()) return x; }
  const grpSpPr = node.child('grpSpPr');
  if (grpSpPr.exists()) { const x = grpSpPr.child('xfrm'); if (x.exists()) return x; }
...
export function parseBaseProps(spNode: SafeXmlNode): Omit<BaseNodeData, 'nodeType'> {
  ...
  const position: Position = { x: emuToPx(off.numAttr('x') ?? 0), y: emuToPx(off.numAttr('y') ?? 0) };
  ...
  return { id, name, position, size, rotation, flipH, flipV, placeholder, hlinkClick, source: spNode };
}
