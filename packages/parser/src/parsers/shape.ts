...
async function parseGraphicFrame(
  ...
  if (uri.includes('chart')) {
    const chartEl = graphicData ? Object.values(graphicData).find(
      (v) => v && typeof v === 'object' && attr(v, 'r:id')
    ) : undefined
...
export async function parseSpTree(
  ...
  for (const grp of grpNodes) {
    elements.push(await parseGrpSp(grp as Record<string, unknown>, rels, zip, slidePath, skipImages))
  }
