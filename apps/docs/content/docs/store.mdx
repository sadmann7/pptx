...
Fitting is a viewport concern: `<Presentation.Viewport autoFit>` calls `fitTo` on mount and on every container resize. A zoom control that offers both a "Fit" entry and explicit levels owns that choice itself, toggling `autoFit` off when a level is picked, otherwise the next resize fits over it.
