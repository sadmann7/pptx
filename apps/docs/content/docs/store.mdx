...
interface PresentationState {
  ...
  zoom: number; // resolved zoom the slide renders at (default: 1)
  zoomLevel: number | "fit"; // the zoom that was asked for (default: "fit")
...
store.load(file, {
  ...
  defaultZoom?: number,  // opens at this level instead of auto-fitting
...
```ts
store.setZoom(1.5)            // pin a level, clamped to [0.1, 4]
store.setZoom("fit")          // let the viewport fit and keep fitting
...
Zoom is two values: `zoom` is the number the slide renders at, and `zoomLevel` is what was asked for. A pinned number survives container resizes; `"fit"` is re-applied on every resize, which is how `Viewport autoFit` knows whether it may touch the zoom.

Only something that knows the container size can turn `"fit"` into a number, so `setZoom("fit")` records the request and leaves `zoom` alone until a mounted `Viewport autoFit` (or your own `fitTo()` call) resolves it.

`zoomLevel` can move while `zoom` does not, so pinning the exact percentage a fit had already produced still counts as pinning it.

The `zoomChange` event reports what produced the change:
...
import {
  ...
  useZoom, // { zoom, zoomLevel, setZoom, zoomIn, zoomOut, fitTo }
} from "@diceui/pptx";
```
