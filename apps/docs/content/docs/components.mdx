...
`autoFit` only fits while the requested zoom is `"fit"`. Pinning a number stops it refitting on resize so the chosen level survives, and `setZoom("fit")` hands the zoom back. Both go through one setter, so a zoom control needs no state of its own:
...
To open a deck at a fixed level instead of fitting, pass `defaultZoom` to `Root`; auto-fit leaves it alone until something asks for `"fit"`.
