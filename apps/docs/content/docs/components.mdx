...
`autoFit` refits on every container resize, so a zoom control that offers explicit levels alongside a "Fit" entry owns that toggle:
...
function Viewer({ file }) {
  ...
  return (
    <Presentation.Root file={file}>
      <ZoomSelect isAutoFit={isAutoFit} onAutoFitChange={setIsAutoFit} />
      <Presentation.Viewport autoFit={isAutoFit}>
        <Presentation.Slide />
      </Presentation.Viewport>
    </Presentation.Root>
  );
...
Picking a level turns `autoFit` off so the next resize leaves it alone; picking "Fit" turns it back on. To open a deck at a fixed level, pass `defaultZoom` to `Root`, which only applies when nothing is auto-fitting over it.
