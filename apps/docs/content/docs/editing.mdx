...
Undo and redo are opt-in, because Ctrl+Z is a shortcut the surrounding app may already own:

```tsx
<Presentation.Selection undoRedoShortcuts />
```

Once on, it is the only shortcut bound above the overlay, so it survives an undo navigating to another slide and works when focus sits on a thumbnail after a keyboard reorder. It still ignores keystrokes aimed at a text field or at anything outside `Presentation.Root`. Leave it off and call `store.undo()` from your own handler to keep full control.
