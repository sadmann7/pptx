...
function FillLayer({ fill }: { fill: BackdropFill }) {
  ...
  return (
    <AbsoluteFill
      style={{
        background: fill.type === "gradient" ? fill.value : undefined,
        backgroundColor: fill.type === "color" ? fill.value : undefined,
      }}
    />
  );
}
...
export function Backdrop({
  ...
  const fillLayer = isValidElement(fill) ? (
    ...
    <FillLayer fill={(fill as BackdropFill) ?? DEFAULT_FILL} />
  );
